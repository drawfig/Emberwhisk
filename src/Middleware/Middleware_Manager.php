...
class Middleware_Manager {
    public function run($data, $fd, $server, $run_type, $routing) {
        ...
        return $this->run_middleware($data, $server, $fd, $this->compile_final_middleware_list($region_middleware_list, $local_middleware_list, $data['message_type']), $run_type, $routing);
    }
    ...
    private function run_middleware($data, $server, $fd, $middleware_list, $run_type, $routing) {
        foreach ($middleware_list as $middleware) {
            ...
            $check = $middleware_instance->run($data, $server, $fd, $routing);
